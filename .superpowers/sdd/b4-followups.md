...
- [x] **cerrado en FF-3, 2026-07-13.** #3 Route-handler behavioral tests for credentials route (401 / JSON-parse 400 /
      ...
      **UPDATED 2026-07-08:** the mappings route now has FOUR handlers — the pass must
      ...
      / `{ ok: true }`).
- [ ] #4 `parseChain(body.chain ?? null)` for sibling consistency (mappings/conflicts use `?? null`).
- [ ] #5 `_req: Request` on credentials GET for idiom consistency with sibling GETs.
- [ ] #6 PUT `update: { username }` leaves `isActive` untouched — reactivation question; document or decide.
- [ ] #7 test password-absence cast alternative: `Object.keys(row!).not.toContain('password')`.

Items #4-#7 → fold into the B4 whole-branch final review.

## Task 7 — code-quality follow-up (logged 2026-06-26, NOT done in Task 7)

- [ ] #1-minor (file-as-field): when `chain`/`fileType` is sent as a multipart File blob
...
- [ ] (optional) rename `lib/portales/chains.ts` → a portal-metadata name now that it also
...
- [ ] #5 (use-portales.ts scaffold hooks): useChainMappings/useChainSuggestions/useChainConflicts
...
- [ ] #8 (chain-upload.tsx): the per-slot label <p> is not associated (htmlFor/id) with the
...
- [ ] **Shared mutation helper:** `confirmDelete` and `confirmRetarget` duplicate ~25 lines of
      the fetch+parse+branch pattern; the file already extracted `postMapping` for the POST.
      Extract e.g. `mutateMapping(method, body)`.
- [ ] **Stale edit-state (theoretical):** if the row in inline-edit is removed by an external
...
- [ ] **ConfirmDialog hardcoded LOADING label:** `components/ui/confirm-dialog.tsx:97` shows
...
- [ ] **deleteMapping presence signal is productId-scoped:** the revert `where` includes
...
- [ ] **retargetMapping nits (11.6a quality):** the pure no-op guard (`new === old`) runs after
...
- [ ] `setConfirming(null)` corre antes del `await onConfirmed`: ventana durante
...
- [ ] `confirmError` stale teórico: si la fila con error desaparece por refetch
